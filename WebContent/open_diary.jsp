	...
	<div class="row"></div>
			...
			<table class ="table table-hover">
				...
				<%
					...
					for(int i=0; i< list.size(); i++){
						if(Integer.parseInt(list.get(i).getDiaryOption())==1){
							open++;
				%>
				<tr>
					<td><%= open %></td> 
					<td><a href="open_view.jsp?diaryID=<%=list.get(i).getDiaryID()%>"> <%= list.get(i).getDiaryTitle() %> </a></td>
		...
		<ul class="pager"> 
			<%
				if(pageNumber != 1) {
			%>
				<li class="previous"><a href="open_diary.jsp?pageNumber=<%=pageNumber-1 %>">이전</a></li>
			<%
				} 
				if(bbsDAO.nextPage(pageNumber+1)) {
			%>
				<li class="next"><a href="open_diary.jsp?pageNumber=<%=pageNumber+1 %>" >다음</a></li>
			<%
				}
			%>
		</ul>
